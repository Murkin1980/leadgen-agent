...
class Lead(Base):
    ...
    consent_status: Mapped[str] = mapped_column(
        String(50), default=ConsentStatus.unknown.value
    )
    ...
    job = relationship(
        "SearchJob", back_populates="leads", foreign_keys=[search_job_id]
    )
    landings = relationship(
        "LandingPage", back_populates="lead", foreign_keys="LandingPage.lead_id"
    )
    stage_history = relationship(
        "LeadStageHistory",
        back_populates="lead",
        foreign_keys="LeadStageHistory.lead_id",
    )
