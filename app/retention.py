...
def get_retention_config() -> dict:
    ...
    return {
        "lead_retention_days": getattr(
            settings, "lead_retention_days", DEFAULT_LEAD_RETENTION_DAYS
        ),
        "message_retention_days": getattr(
            settings, "message_retention_days", DEFAULT_MESSAGE_RETENTION_DAYS
        ),
        "event_retention_days": getattr(
            settings, "event_retention_days", DEFAULT_EVENT_RETENTION_DAYS
        ),
    }
...
def purge_preview(db: Session) -> dict:
    ...
    old_events = (
        db.query(OutreachEvent).filter(OutreachEvent.created_at < event_cutoff).count()
    )
...
def purge_execute(db: Session, actor: str = "system", dry_run: bool = False) -> dict:
    ...
    deleted_events = 0
    old_events = (
        db.query(OutreachEvent).filter(OutreachEvent.created_at < event_cutoff).all()
    )
