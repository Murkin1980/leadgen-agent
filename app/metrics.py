...
messages_sent_total = Counter(
    "leadgen_messages_sent",
    "Total outbound messages sent",
)
messages_delivered_total = Counter(
    "leadgen_messages_delivered", "Messages confirmed delivered"
)
messages_read_total = Counter("leadgen_messages_read", "Messages confirmed read")
messages_failed_total = Counter(
    "leadgen_messages_failed", "Messages that failed delivery"
)
messages_retried_total = Counter("leadgen_messages_retried", "Messages retried")
messages_dead_letter_total = Counter(
    "leadgen_messages_dead_letter", "Messages in dead letter queue"
)
webhook_duplicate_total = Counter(
    "leadgen_webhook_duplicates", "Duplicate webhook events dropped"
)
webhook_processed_total = Counter(
    "leadgen_webhook_processed", "Webhook events successfully processed"
)
api_request_latency = Histogram(
    "leadgen_api_request_latency_seconds", "API request latency in seconds"
)
...
landing_pages_published = Counter(
    "leadgen_landing_pages_published", "Landing pages published"
)
...
def _format_metric(name: str, m: Counter | Histogram) -> str:
    lines = [
        f"# HELP {m.help}",
        f"# TYPE {name} gauge" if isinstance(m, Counter) else f"# TYPE {name} summary",
    ]
