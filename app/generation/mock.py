...
class MockTextGenerationAdapter:
    ...
    def generate(self, context: GenerationContext) -> GeneratedProfile:
        ...
        services = (
            context.services or ["Услуга 1", "Услуга 2"]
            if lang == "ru"
            else ["Қызмет 1", "Қызмет 2"]
        )

        claims = [
            GeneratedProfileClaim(
                text=context.company_name, source_field="company_name", verified=True
            ),
            GeneratedProfileClaim(
                text=f"Город: {context.city}", source_field="city", verified=True
            ),
        ]
        if phone:
            claims.append(
        ...
        meta_title = f"{context.category} — {context.company_name}"
        ...
        profile_data = {
            ...
            if lang == "ru"
            else ["Артықшылық 1", "Артықшылық 2"],
            "work_stages": [
