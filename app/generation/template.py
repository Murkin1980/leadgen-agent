...
FAQS_RU = [
    {
        ...
        "answer": "Срок изготовления зависит от сложности проекта и составляет от 7 до 21 рабочего дня.",
    },
    {
        "question": "Как происходит замер?",
        "answer": "Наш специалист выезжает на объект, делает точные замеры и согласовывает детали.",
    },
    {
        "question": "Предоставляете ли вы гарантию?",
        "answer": "Да, мы предоставляем гарантию на все изделия.",
    },
    {
        "question": "Можно ли посмотреть примеры работ?",
        "answer": "Да, мы предоставляем фото выполненных проектов.",
    },
]

FAQS_KK = [
    {
        "question": "Өндіріс қанша уақыт алады?",
        "answer": "Өндіріс мерзімі жобаның күрделілігіне байланысты және 7-ден 21 жұмыс күніне дейін.",
    },
    {
        "question": "Өлшеу қалай жүреді?",
        "answer": "Біздің маман нысанға шығып, дәл өлшеу жасайды және мәселелерді келіседі.",
    },
    {
        "question": "Кепілдік бересіз бе?",
        "answer": "Иә, біз барлық өнімдерге кепілдік береміз.",
    },
    {
        "question": "Жұмыс үлгілерін көруге бола ма?",
        "answer": "Иә, біз орындалған жобалардың фотоларын береміз.",
    },
]
...
class TemplateTextGenerationAdapter:
    def generate(self, context: GenerationContext) -> GeneratedProfile:
        ...
        whatsapp_url = context.whatsapp_url or (
            make_whatsapp_url(phone) if phone else ""
        )
        ...
        if lang == "kk":
            ...
            hero_subtitle = (
                f"{context.company_name} — жеке өлшемдер бойынша жиһаз өндіру"
            )
            ...
            hero_subtitle = (
                f"{context.company_name} — изготовим мебель по индивидуальным размерам"
            )
        ...
        claims = [
            GeneratedProfileClaim(
                text=meta_title, source_field="company_name", verified=True
            ),
            GeneratedProfileClaim(
                text=f"Город: {city_label}", source_field="city", verified=True
            ),
        ]
        if phone:
            claims.append(
                GeneratedProfileClaim(
                    text=f"Телефон: {phone}", source_field="phone", verified=True
                )
            )
        ...
        profile_data = {
            ...
            "services": [
                {
                    "title": s,
                    "description": f"Профессиональное изготовление: {s.lower()}"
                    if lang == "ru"
                    else f"Кәсіби өндіріс: {s.lower()}",
                }
                for s in services
            ],
