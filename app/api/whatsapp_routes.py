        ...
        for change in entry.get("changes", []):
            ...
            for status in value.get("statuses", []):
                ...
                msg = (
                    ...
                    .filter(OutreachMessage.provider_message_id == provider_id)
                    .first()
                )
                ...
                if (
                    ...
                    .filter(OutreachEvent.provider_event_id == event_key)
                    .first()
                ):
                    continue
                msg.status = state
                ...
                db.add(
                    OutreachEvent(
                        ...
                        payload_json=json.dumps({"status": state}),
                    )
                )
                changed += 1
            for item in value.get("messages", []):
                ...
                lead = (
                    db.query(Lead)
                    .filter((Lead.whatsapp == phone) | (Lead.phone == phone))
                    .first()
                )
                ...
                inbound = InboundMessage(
                    ...
                    text_body=text,
                    raw_metadata_json=json.dumps({"type": message_type}),
                )
                db.add(inbound)
                if lead:
                    now = datetime.now(UTC)
                    lead.last_inbound_at = now
                    lead.service_window_expires_at = now + timedelta(
                        hours=settings.whatsapp_service_window_hours
                    )
                    lead.stage = "replied"
                    pending = (
                        db.query(OutreachMessage)
...
def list_inbound_messages(
    ...
    if lead_id:
        query = query.filter(InboundMessage.lead_id == lead_id)
    if status:
        query = query.filter(InboundMessage.status == status)
    return (
        query.order_by(InboundMessage.received_at.desc())
        .offset(offset)
        .limit(min(limit, 500))
        .all()
    )
...
def update_consent(
    lead_id: int, payload: ConsentPayload, db: Session = Depends(get_db)
):
    ...
    lead.consent_recorded_at = datetime.now(UTC)
    if payload.consent_status in {
        ConsentStatus.withdrawn.value,
        ConsentStatus.blocked.value,
    }:
